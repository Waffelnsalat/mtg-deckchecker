...
| Change Scryfall resolution | `src/scryfall.ts` | Collection batching, cache, throttling, Retry-After handling, fuzzy fallback, public card shape. |
...
| Change power/bracket score | `src/powerAnalysis.ts`, `src/bracketAnalysis.ts` | Final scoring, mulligan leverage, soft bracket boundaries, and Commander bracket interpretation. |
| Change card recommendations | `src/recommendationAnalysis.ts` | Add/down suggestions, bracket-blocking dimensions, and external recommendation context. |
| Change weakness read | `src/weaknessAnalysis.ts` | Strategy weaknesses and opposing deck profiles that pressure the submitted deck. |
| Change website layout | `public/index.html` | Static HTML structure, modals, result sections, and element ids. |
| Change website styling | `public/styles.css` | Visual design and responsive layout. |
| Change browser behavior | `public/app.js` plus focused `public/*.js` modules | UI events, API calls, target bracket prompt, strategy recalculation, and rendering analysis results. |
...
4. `src/powerAnalysis.ts` combines module scores and hand-selection leverage.
5. `src/bracketAnalysis.ts` turns the power read, hard rules signals, target bracket, and soft gap budgets into a bracket.
6. `src/recommendationAnalysis.ts` builds card suggestions from weak topics, bracket blockers, EDHREC, and Recommander.
7. `src/weaknessAnalysis.ts` builds opposing-plan and hate-profile notes.
8. The API returns `{ document, validation, sources, analysis }` to the browser UI.
...
| Recommendations | `src/recommendationAnalysis.ts` |
| Opposing deck weaknesses | `src/weaknessAnalysis.ts` |
