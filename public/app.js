...
function initializeCardBreakdownControls() {
  cardBreakdownSearch?.addEventListener("input", () => {
    cardBreakdownSearchText = cardBreakdownSearch.value;
    rerenderCardBreakdown();
  });

  cardBreakdownSectionFilter?.addEventListener("change", () => {
    cardBreakdownSectionMode = cardBreakdownSectionFilter.value || "all";
    rerenderCardBreakdown();
  });

  cardBreakdownRoleFilter?.addEventListener("change", () => {
    cardBreakdownRoleMode = cardBreakdownRoleFilter.value || "all";
    rerenderCardBreakdown();
  });

  cardBreakdownSort?.addEventListener("change", () => {
    cardBreakdownSortMode = cardBreakdownSort.value || "value_desc";
    rerenderCardBreakdown();
  });
}

function rerenderCardBreakdown() {
  if (!currentAnalysisDocument || !currentAnalysisSnapshot) {
    return;
  }

  renderCardBreakdown(currentAnalysisDocument, currentAnalysisSnapshot);
}
...
function renderCardBreakdown(deckDocument, analysis) {
  if (!cardBreakdownBody) {
    return;
  }

  const rows = buildCardBreakdownRows(deckDocument, analysis);
  const filteredRows = sortCardBreakdownRows(filterCardBreakdownRows(rows));
  ...
  if (cardBreakdownCount) {
    cardBreakdownCount.textContent = `${filteredRows.length} entries / ${totalEntries} cards`;
  }

  if (cardBreakdownSummary) {
    cardBreakdownSummary.textContent =
      rows.length > 0
  ...
  const renderedRows = filteredRows.map((row) => createCardBreakdownRow(row));
  if (renderedRows.length === 0) {
    ...
    cardBreakdownBody.replaceChildren(emptyState);
    return;
  }

  cardBreakdownBody.replaceChildren(...renderedRows);
}

function buildCardBreakdownRows(deckDocument, analysis) {
  const roleIndex = buildCardRoleIndex(analysis);

  return (deckDocument?.result?.resolvedCards ?? [])
    .filter((deckCard) => ["commander", "mainboard", "companion"].includes(deckCard.section))
    .map((deckCard, index) => {
      const lookupKey = getCardBreakdownLookupKey(deckCard);
      const mergedRoles = mergeCardBreakdownRoles(
        roleIndex.get(lookupKey) ?? [],
        roleIndex.get(getCardBreakdownNameKey(deckCard.card.name)) ?? [],
      );
      const displayRoles = limitCardBreakdownRoles(mergedRoles);
      ...
      return {
        ...
        imageUrl: getCardImageUrl(deckCard.card),
...
function filterCardBreakdownRows(rows) {
  const query = normalizeText(cardBreakdownSearchText);

  return rows.filter((row) => {
    if (cardBreakdownSectionMode !== "all" && row.section !== cardBreakdownSectionMode) {
      return false;
    }

    const impact = getCardBreakdownImpact(row);
    if (cardBreakdownRoleMode === "tagged" && row.roles.length === 0) {
      return false;
    }
    if (cardBreakdownRoleMode === "none" && row.roles.length > 0) {
      return false;
    }
    if (["strong", "useful", "minor"].includes(cardBreakdownRoleMode) && impact.tier !== cardBreakdownRoleMode) {
      return false;
    }
    ...
    return getCardBreakdownSearchText(row).includes(query);
  });
}

function sortCardBreakdownRows(rows) {
  const sortedRows = [...rows];
  sortedRows.sort((left, right) => {
    switch (cardBreakdownSortMode) {
      case "value_asc":
        return getCardBreakdownOverall(left.roles).score - getCardBreakdownOverall(right.roles).score ||
      ...
      case "value_desc":
      default:
        return getCardBreakdownOverall(right.roles).score - getCardBreakdownOverall(left.roles).score ||
          left.name.localeCompare(right.name);
    }
  });
  return sortedRows;
...
function getCardBreakdownSearchText(row) {
...
function buildCardRoleIndex(analysis) {
  const roleIndex = new Map();

  addTaggedCardsToRoleIndex(roleIndex, analysis?.commander?.taggedCommanders, {
    label: "Commander",
    valueKey: "roleValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.gameChangers?.taggedCards, {
    label: "Game Changer",
    valueKey: null,
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.landBase?.taggedCards, {
    label: "Land Base",
    valueKey: "landValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.ramp?.taggedCards, {
    label: "Ramp",
    valueKey: "rampValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.draw?.taggedCards, {
    label: "Card Flow",
    valueKey: "drawValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.consistency?.taggedCards, {
    label: "Consistency",
    valueKey: "consistencyValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.removal?.taggedCards, {
    label: "Removal",
    valueKey: "removalValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.spellInteraction?.taggedCards, {
    label: "Spell Interaction",
    valueKey: "interactionValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.protection?.taggedCards, {
    label: "Protection",
    valueKey: "protectionValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.recursion?.taggedCards, {
    label: "Recursion",
    valueKey: "recursionValue",
  });
  addTaggedCardsToRoleIndex(roleIndex, analysis?.winConditions?.taggedCards, {
    label: "Wincon",
    valueKey: "finisherValue",
  });
  addAdvancedTaggedCardsToRoleIndex(roleIndex, analysis?.advancedRoles?.taggedCards);

  addNameListToRoleIndex(roleIndex, analysis?.strategy?.mainStrategy?.keyCards, "Strategy", 1);
  for (const entry of analysis?.strategy?.subStrategies ?? []) {
    addNameListToRoleIndex(roleIndex, entry.keyCards, "Substrategy", 0.5);
  }
  addNameListToRoleIndex(roleIndex, analysis?.winStrategy?.primaryPlan?.keyCards, "Win Plan", 1);
  for (const combo of analysis?.winConditions?.combos?.exact ?? []) {
    addNameListToRoleIndex(roleIndex, combo.cardNames, "Combo Line", combo.comboValue ?? 1);
  }

  return roleIndex;
}

function addAdvancedTaggedCardsToRoleIndex(roleIndex, taggedCards = []) {
  for (const taggedCard of taggedCards ?? []) {
    ...
    for (const hit of taggedCard.hits ?? []) {
      ...
      const label = formatTagLabel(hit.tag);
      if (isCardRoleAlreadyCovered(roleIndex, taggedCard, label)) {
        continue;
      }

      addCardRole(roleIndex, taggedCard.name, taggedCard.section, {
        label,
...
function isCardRoleAlreadyCovered(roleIndex, taggedCard, label) {
  const keys = [
    `${taggedCard.section}:${normalizeText(taggedCard.name)}`,
    getCardBreakdownNameKey(taggedCard.name),
  ];
...
function addFallbackTaggedCardsToRoleIndex(roleIndex, taggedCards = [], options) {
  for (const taggedCard of taggedCards ?? []) {
    if (!taggedCard.name || !taggedCard.section) {
      continue;
    }

    const exactKey = `${taggedCard.section}:${normalizeText(taggedCard.name)}`;
    const looseKey = getCardBreakdownNameKey(taggedCard.name);
    if ((roleIndex.get(exactKey) ?? []).length > 0 || (roleIndex.get(looseKey) ?? []).length > 0) {
      continue;
    }

    const tags = getTaggedCardHitTags(taggedCard);
...
function addTaggedCardsToRoleIndex(roleIndex, taggedCards = [], options) {
  for (const taggedCard of taggedCards ?? []) {
    const tags = getTaggedCardHitTags(taggedCard);
    const value = options.valueKey ? Number(taggedCard[options.valueKey] ?? 0) : 1;
    addCardRole(roleIndex, taggedCard.name, taggedCard.section, {
...
function addNameListToRoleIndex(roleIndex, names = [], label, value) {
  for (const name of names ?? []) {
    addCardRole(roleIndex, name, null, {
      label,
      value,
      tags: [],
    });
...
function addCardRole(roleIndex, name, section, role) {
  if (!name) {
    return;
  }

  const displayRole = getCardBreakdownDisplayRole(role);
  if (shouldHideLowSignalCardBreakdownRole(displayRole)) {
    return;
  }

  const keys = section ? [`${section}:${normalizeText(name)}`] : [getCardBreakdownNameKey(name)];
...
function mergeCardBreakdownRoles(...roleGroups) {
  const merged = [];
...
function getTaggedCardHitTags(taggedCard) {
...
function getCardBreakdownDisplayRole(role) {
  const rawLabel = String(role.label ?? "");
  const rawTags = (role.tags ?? []).map((tag) => String(tag));
  const normalizedParts = [rawLabel, ...rawTags].map(normalizeText);
  ...
  return {
    ...
    tags: Array.from(new Set([rawLabel, ...rawTags].filter(Boolean).map(formatTagLabel))),
  };
}

function shouldHideLowSignalCardBreakdownRole(role) {
  if (role.label !== "Land Base") {
    return false;
  }

  const tags = role.tags ?? [];
  return tags.length > 0 && tags.every((tag) => CARD_BREAKDOWN_LOW_SIGNAL_LAND_TAGS.has(tag));
}

function limitCardBreakdownRoles(roles) {
  return [...roles]
    .sort(compareCardBreakdownDisplayRoles)
    .slice(0, CARD_BREAKDOWN_MAX_ROLES);
}

function compareCardBreakdownDisplayRoles(left, right) {
  const synergyOrder = Number(!isCardBreakdownSynergyRole(left)) - Number(!isCardBreakdownSynergyRole(right));
  if (synergyOrder !== 0) {
    return synergyOrder;
  }
  ...
  const priorityOrder = getCardBreakdownRolePriority(left.label) - getCardBreakdownRolePriority(right.label);
  if (priorityOrder !== 0) {
...
function isCardBreakdownSynergyRole(role) {
  return CARD_BREAKDOWN_SYNERGY_LABELS.has(role.label);
}

function getCardBreakdownRolePriority(label) {
  const index = CARD_BREAKDOWN_ROLE_PRIORITY.indexOf(label);
  return index === -1 ? CARD_BREAKDOWN_ROLE_PRIORITY.length : index;
}

function createCardBreakdownRow(row) {
  ...
  sectionPill.textContent = `${formatTagLabel(row.section)} x${row.quantity}`;
  ...
  facts.append(
    createCardBreakdownFact("MV", formatOneDecimal(row.manaValue)),
    createCardBreakdownFact("Type", row.typeLine),
  );

  const roles = document.createElement("div");
  roles.className = "card-breakdown-role-list";
  if (row.roles.length > 0) {
    roles.append(...row.roles.map(createCardBreakdownRoleChip));
  } else {
...
function getCardBreakdownImpact(row) {
  const overall = getCardBreakdownOverall(row.roles);
  if (overall.score <= 0) {
...
function getCardBreakdownOverall(roles) {
  if (!roles.length) {
  ...
  const normalizedRoles = roles
    .map((role) => {
      const scale = getCardBreakdownRoleScale(role);
  ...
  const score = Math.min(10, roundTo((scoreBase / 1.9) * 10, 1));
  return {
    score,
    scale: 10,
...
function getCardBreakdownTierRank(tier) {
  switch (tier) {
    case "strong":
      return 3;
    case "useful":
...
function getCardBreakdownRoleScale(role) {
...
function createCardBreakdownFact(label, value) {
...
function createCardBreakdownRoleChip(role) {
  const chip = document.createElement("span");
...
function getCardBreakdownLookupKey(deckCard) {
  return `${deckCard.section}:${normalizeText(deckCard.card.name)}`;
}

function getCardBreakdownNameKey(name) {
  return `card:${normalizeText(name)}`;
}
